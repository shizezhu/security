...
<title>小石权限系统 -新增菜单</title>
<script type="text/javascript">
	$(function() {
		$("select").chosen();
		$.myAjaxForm("form", {
			url : "${pageContext.request.contextPath}/menu/smSave",
			success : function(result) {
				if (result.success) {
					...
					var currMenu = result.data;
					var currNode = null;
					do {
						currMenu = currMenu.parentMenu;
						currNode = jstree.get_node(currMenu == null ? "0" : currMenu.id)
					} while (!currNode);
					jstree.refresh_node(currNode);
					jstree.select_node(currNode, false, false);
					jstree.open_node(currNode);
					$("form input[type='text']").not("#parentName").val("");
				} else {
					...
					<div class="ibox-content">
						<form class="form-horizontal m-t">
							<div class="form-group">
								...
								<div class="col-sm-8">
									<div class="radio radio-primary radio-inline">
                                        <input type="radio" id="type_directory" value="1" name="type" <c:if test="${menu == null}">checked="checked"</c:if> >
                                        <label style="color: #1c84c6;" for="type_directory">目录</label>
                                    </div>
                                    <div class="radio radio-success radio-inline">
                                        <input type="radio" id="type_menu" value="2" name="type" <c:if test="${menu != null && menu.type == 1}">checked="checked"</c:if> >
                                        <label style="color: #1ab394;" for="type_menu">页面</label>
                                    </div>
                                    <div class="radio radio-warning radio-inline">
                                        <input type="radio" id="type_button" value="3" name="type" <c:if test="${menu != null && (menu.type == 2 || menu.type == 3)}">checked="checked"</c:if> >
							...
							<div class="form-group">
								...
								<div class="col-sm-8">
									<input name="parentId" id="parentId" type="hidden" value="${menu == null ? 0 : menu.id}"/>
									<input name="parentName" id="parentName" type="text" 
									class="form-control" value="${menu == null ? '顶级' : menu.name}" 
									readonly="readonly" style="cursor: pointer;" onclick="menutree()">
								</div>
							</div>
							<div class="form-group" <c:if test="${menu == null}">style="display: none;"</c:if> >
							...
							<div class="form-group" <c:if test="${menu == null}">style="display: none;"</c:if> >
								...
								<div class="col-sm-8">
									<input name="authIden" id="authIden" type="text" class="form-control" value="" placeholder="例如:user:save,多个用,隔开">
								</div>
							</div>
							<div class="form-group" <c:if test="${menu != null}">style="display: none;"</c:if> >
							...
							<div class="form-group" <c:if test="${menu != null && (menu.type == 2 || menu.type == 3)}">style="display: none;"</c:if> >
								<label class="col-sm-3 control-label">排序：</label>
								<div class="col-sm-8">
									<input name="sort" id="sort" type="number" class="form-control" value="1">
								</div>
							</div>
						</form>
					</div>
