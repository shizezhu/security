...
<script type="text/javascript">
	$(function() {
		...
		$.myAjaxForm("form", {
			url : "${pageContext.request.contextPath}/user/smUpdate",
				...
				<div class="ibox float-e-margins">
					<div class="ibox-content">
						<form class="form-horizontal m-t">
							<input type="hidden" name="id" value="${up_user.id }"/>
							<div class="form-group">
								<label class="col-sm-3 control-label">用户名：</label>
								<div class="col-sm-8">
									<input name="username" type="text"
										class="form-control" value="${up_user.username }">
								</div>
							</div>
							<div class="form-group">
								...
								<div class="col-sm-8">
									<input name="realname" type="text"
										class="form-control" value="${up_user.realname }">
								</div>
							</div>
							<div class="form-group">
							...
							<div class="form-group">
								...
								<div class="col-sm-8">
									<select name="status" style="width: 95%;"
										tabindex="2">
										<option ${up_user.status == true ? 'selected' : ''} value="1">有效</option>
										<option ${up_user.status == false ? 'selected' : ''} value="0">无效</option>
									</select>
								</div>
							</div>
							<div class="form-group">
								...
								<div class="col-sm-8">
									<c:forEach items="${roleList }" var="role">
										<label class="checkbox-inline i-checks">
											<input ${roleIdMap[role.id] != null ? 'checked' : '' } type="checkbox" name="role" value="${role.id }">${role.name}
										</label>
									</c:forEach>
								</div>
							</div>
